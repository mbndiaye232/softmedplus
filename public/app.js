...
// ----------------------------------------------------------------------------
// Guide des modules recents
// ----------------------------------------------------------------------------
// Chacun de ces modules vit a l'interieur d'un ecran existant : le mode de
// consultation est un champ du formulaire de rendez-vous, a plus de 1700 pixels
// du haut de l'agenda ; les alertes d'interaction n'apparaissent qu'une fois une
// ordonnance commencee. Rien ne les annonce, et ils passent donc pour absents.
// Ce guide les nomme et emmene directement au bon endroit.

// Fait defiler jusqu'a un element et l'entoure brievement, pour que l'oeil le
// retrouve apres le defilement.
function mettreEnEvidence(element) {
  if (!element) return;
  element.scrollIntoView({ behavior: 'smooth', block: 'center' });
  const ancien = element.style.boxShadow;
  element.style.transition = 'box-shadow 0.3s';
  element.style.boxShadow = '0 0 0 4px rgba(245, 158, 11, 0.55)';
  setTimeout(() => { element.style.boxShadow = ancien; }, 3200);
}
...
async function allerVersModule(cle) {
  if (cle === 'teleconsultation' || cle === 'rappels') {
    ...
    const champ = document.getElementById(cle === 'rappels' ? 'book-reminder-enabled' : 'book-consultation-mode');
    if (cle === 'teleconsultation') {
      const select = document.getElementById('book-consultation-mode');
      if (select) select.value = 'TELECONSULTATION';
    }
    mettreEnEvidence(champ ? champ.closest('.form-group') || champ : null);
    showToast(
      cle === 'teleconsultation'
        ? 'Mode « Téléconsultation » présélectionné : complétez le rendez-vous et enregistrez.'
        : 'Réglez ici le délai et le canal du rappel avant d\'enregistrer le rendez-vous.',
      'info'
    );
    return;
  }
  ...
  if (cle === 'interactions') {
    ...
    showToast('Ouvrez un dossier patient, puis commencez une ordonnance : les alertes apparaissent dès le deuxième médicament.', 'info');
    return;
  }
  ...
  if (cle === 'copilote') {
    ...
    mettreEnEvidence(document.getElementById('admin-copilot-card'));
    return;
  }
